...
function themeColor(a) {
  ...
  var themecElements = document.querySelectorAll('.themec');

  themecElements.forEach(function(element) {
    element.setAttribute("data-color", color);
  });

  // Set cookie to store the selected color
  document.cookie = "selectedColor=" + color + "; expires=Thu, 31 Dec 2099 23:59:59 UTC; path=/";
}
...
// On page load, set the theme color based on the stored cookie value
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (storedColor) {
    ...
    themecElements.forEach(function(element) {
      element.setAttribute("data-color", storedColor);
    });
  }
});
...
// On page load, set the dark mode based on the stored cookie value
document.addEventListener("DOMContentLoaded", function() {
  var storedDarkMode = getCookie("darkMode");
  if(storedDarkMode == "1"){
    const body = document.getElementsByTagName('body')[0];
    const hr = document.querySelectorAll('div:not(.sidenav) > hr');
    const sidebar = document.querySelector('.sidenav');
    ...
    const cardNavLinksIcons = document.querySelectorAll('.card .nav .nav-link i');
    const cardNavSpan = document.querySelectorAll('.card .nav .nav-link span');
    const dmt = document.querySelector('#dark-version');
